...
class Trajectory(object):
    ...
    def __init__(self, msg: TrajMsg, L=0.257):
        ...
        self.ref_traj_f = Curve(x=x_f, y=y_f, k=3)
        
        # use cubic spline to fit time dependent v, psi, pos (x, y)
        self.ref_x_f = CubicSpline(self.t, np.array(msg.x))
        self.ref_y_f = CubicSpline(self.t, np.array(msg.y))
        ...
        theta_sample = np.linspace(0, 1, 5000, endpoint=False)*self.length
    ...
    def get_closest_pt(self, x, y, psi, t):
        ...
        # error = np.sin(slope)*(x-closest_pt[0])-np.cos(slope)*(y-closest_pt[1])
        error = np.sin(psi)*(x-closest_pt[0])-np.cos(psi)*(y-closest_pt[1])
        return slope, error
...
class Tracking_Stanley(object):
    def __init__(self,
        ...
        self.course_sub = rospy.Subscriber(
            ref_traj_topic, TrajMsg, self.ref_traj_sub_callback
        )        

        if self.vicon_pose:
            self.pose_sub = rospy.Subscriber(
                pose_topic, TransformStamped, self.vicon_sub_callback
            )
        else:
            self.pose_sub = rospy.Subscriber(
                pose_topic, Odometry, self.odom_sub_callback, queue_size = 10
            )
    ...
    def odom_sub_callback(self, odomMsg):
        ...
        if v is not None:
            ...
            if cur_traj is not None:
                ...
                delta = self.lateral_control(cur_traj, x_f, y_f, psi, v, cur_t)
                d = self.longitudial_control(cur_traj.get_pos_ref(cur_t), [x_f, y_f], v, ref_v, psi, dt)
                d = d  + abs(delta)*0.01
                # constraint for acceleration
                d = np.clip(d - self.prev_d, -0.5*max(v, 0.2), 0.5*max(v, 0.2)) + self.prev_d
    ...
    def vicon_sub_callback(self, msg):
        ...
        if self.prev_t is not None:
            ...
            cur_traj = self.traj_buffer.readFromRT()
            cur_v_r = (((x_r - self.prev_state[0])**2+(y_r - self.prev_state[1])**2)**0.5)/dt
            if cur_traj is not None:    
                ref_v = cur_traj.get_v_ref(cur_t)
                
                self.dyn_reconfig_lock.acquire()
                delta = self.lateral_control(cur_traj, x_f, y_f, cur_psi, cur_v_r, cur_t)
                d = self.longitudial_control(cur_traj.get_pos_ref(cur_t), [x_f, y_f], cur_v_r, ref_v, dt)
    ...
    def lateral_control(self, cur_traj, x_f, y_f, psi, v, cur_t):
        slope, error = cur_traj.get_closest_pt(x_f, y_f, psi, cur_t)

        # theta_e corrects the heading error
        theta_e = normalize_angle(normalize_angle(slope) - psi)
        # theta_d contouring error
        theta_d = np.arctan2(self.p_lat * error, v)
    ...
    def longitudial_control(self, pos_ref, cur_pos, cur_v, ref_v, cur_psi, dt):
        ...
        is_front = False
        if abs(np.arctan2(np.sin(heading_angle - cur_psi), np.cos(heading_angle - cur_psi))) < np.pi*0.5:
            is_front = True
