...
class TrackingVisualizeNode:
    ...
    def start_listening(self):
        ...
        rospy.Subscriber("/simple_trajectory_topic", TrajMsg, self.callback_trajectory)
        rospy.Subscriber("/simple_trigger", Bool, self.trigger_sub_callback)
    
    def trigger_sub_callback(self, data):
        self.t_0 = rospy.Time.now()
        if self.traj_received:
            self.course.t_0 = self.t_0.to_sec()
            self.triggered = True
        else:
            print("Trajectory is not yet received")
    ...
    def callback_odom(self, data):
        ...
        # x_value, y_value = data.pose.position.x, data.pose.position.y
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown() and listener.course is not None:
        display.clear_output(wait = True)
        display.display(plt.gcf())
        plt.clf()
        listener.lock.acquire()
        plt.plot(listener.course.data[0], listener.course.data[1], "-r", label="course", linewidth = 1.0)
        try:
            plt.scatter(listener.ref_pos[0], listener.ref_pos[1], color="k", zorder=10, alpha=0.6)
            plt.plot(listener.x_traj, listener.y_traj, linewidth = 2.0, alpha=0.6)
            plt.plot([listener.x_traj[-1], listener.directional_arrow[0]], [listener.y_traj[-1], listener.directional_arrow[1]], "g", alpha = 0.2, linewidth=2.0)
        except IndexError:
            pass
