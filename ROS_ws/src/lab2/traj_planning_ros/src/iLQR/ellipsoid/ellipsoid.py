...
class Ellipsoid():
  ...
  def __init__(
      self, q=np.array([]), Q=np.array([[], []]), auto_sym=True, psd_tol=1e-15
  ):
    """
    ...
    n = q.size
    if Q.shape[1] != n:
      raise ValueError("[ellReach] dimensions of the q and Q do not match.")
    if n > 0:
  ...
  def display(self):
    ...
    if self.dim() == 0:
      ...
      if self.rank() < self.dim():
        print(
            "Degenerate (rank %d) ellipsoid in R^%d.\n" %
            (self.rank(), self.dim())
        )
      else:
  ...
  def regularize(self, abs_tol=1e-7):
    ...
    if n > r:
      ...
      R1 = np.concatenate((np.zeros((r, r)), np.zeros((r, n - r))), axis=1)
      R2 = np.concatenate((np.zeros((n - r, r)), abs_tol * np.eye(n - r)),
                          axis=1)
  ...
  def volume(self):
    ...
    if np.mod(N, 2) > 0:
      ...
      k = N / 2
      S = ((2**(2*k + 1)) *
           (np.pi**k) * np.math.factorial(k)) / np.math.factorial(2*k + 1)
