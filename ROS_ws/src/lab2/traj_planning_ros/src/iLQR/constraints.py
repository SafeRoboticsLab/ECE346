...
class Constraints:
  ...
  def get_derivatives(
    ...
    c_x_rd, c_xx_rd = self._road_boundary_derivatie(states, closest_pt, slope)
  ...
  def _lat_accec_bound_derivative(
    ...
    c_xx[2, 2, :] = self.q2_lat**2 * (b_ub+b_lb) * da_dx**2 + self.q2_lat * (
        b_ub-b_lb
    ) * da_dxx
    c_uu[1, 1, :] = self.q2_lat**2 * (b_ub+b_lb) * da_du**2 + self.q2_lat * (
        b_ub-b_lb
    ) * da_duu

    c_ux[1, 2, :] = (
        self.q2_lat**2 * (b_ub+b_lb) * da_dx * da_du + self.q2_lat *
        (b_ub-b_lb) * da_dux
    )
  ...
  def barrier_function(
      self, q1: float, q2: float, c: np.ndarray, c_dot: np.ndarray
  ) -> np.ndarray:
    '''
    c = [n] array
    c_dot = [dxn] array
    ...
    b_ddot = np.einsum(
        'n,abn->abn', (q2**2) * b, np.einsum('an,bn->abn', c_dot, c_dot)
    )
    return b_dot, b_ddot
